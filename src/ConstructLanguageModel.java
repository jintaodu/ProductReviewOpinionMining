...
public class ConstructLanguageModel {
	...
	public ConstructLanguageModel(String reviewpath)
	{
	...
	private boolean ispunctuation(String word) {
		if (word.contains("_,") || word.contains("_.")
				|| word.contains("_:") || word.contains("_''")||word.contains("_``"))
			return true;
		return false;
	}
	...
	public void constructLanguageModelfromreview(String path) throws IOException {
		String taggedcorpusdir = path;
		
		for (int i = 0; i < filename.length; ++i)// 对XML格式的review循环处理
		{
			File f = new File(taggedcorpusdir+"\\"+filename[i]);
			if(!(f.isDirectory()))
			{
				DomainCount.clear();
				...
				while((line = br.readLine()) != null)
				{
					...
					for(String word:words)
					{
						...
						if(trimedword.length() != 0)
						{
							...
							if(isnounphrase(trimedword))
							{//System.out.println("21341435234564");
								insertdomaincount(trimedword.substring(0,trimedword.indexOf('_'))+"_NN");
								insertcollectioncount(trimedword.substring(0,trimedword.indexOf('_'))+"_NN");
							}else if(!ispunctuation(trimedword))
							{//System.out.println("11111111111111111111111111111");
								insertdomaincount(trimedword.substring(0,trimedword.indexOf('_')));
								insertcollectioncount(trimedword.substring(0,trimedword.indexOf('_')));
							}
						}
					}//end of for
					...
					for (String word : words) {// 查找所有的名词和名词短语
						if (word.trim().length() != 0) {
							if (isnounphrase(word))
								...
								{
									insertdomaincount(nouns.trim()+"_NN");
									insertcollectioncount(nouns.trim()+"_NN");
								}								
								nouns = "";
							}
						}// end of if
	...
	public static void main(String[] args) throws IOException {
		...
		String reviewpath = ".\\TaggedResults";
		ConstructLanguageModel ConLM = new ConstructLanguageModel(reviewpath);
